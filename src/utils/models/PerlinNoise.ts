...
export class PerlinNoise {
    ...
    noise3d(x, y, z) {
      ...
      var p0 = this.perm[X] + Y, p00 = this.perm[p0] + Z, p01 = this.perm[p0 + 1] + Z,
          p1 = this.perm[X + 1] + Y, p10 = this.perm[p1] + Z, p11 = this.perm[p1 + 1] + Z;
      return lerp(fz,
        lerp(fy, lerp(fx, grad3d(this.perm[p00], x, y, z), grad3d(this.perm[p10], x-1, y, z)),
                 lerp(fx, grad3d(this.perm[p01], x, y-1, z), grad3d(this.perm[p11], x-1, y-1,z))),
        lerp(fy, lerp(fx, grad3d(this.perm[p00 + 1], x, y, z-1), grad3d(this.perm[p10 + 1], x-1, y, z-1)),
                 lerp(fx, grad3d(this.perm[p01 + 1], x, y-1, z-1), grad3d(this.perm[p11 + 1], x-1, y-1,z-1))));
    }
    ...
    noise2d(x, y) {
      ...
      return lerp(fy,
        ...
        lerp(fx, grad2d(this.perm[p0 + 1], x, y-1), grad2d(this.perm[p1 + 1], x-1, y-1)));
    }

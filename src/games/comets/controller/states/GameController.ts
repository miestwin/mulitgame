...
export class GameController extends Phaser.State {
  ...
  preload() {
    ...
    Network.onEndGame(data => {
      const message = data == (<any>this.game.state).id ? "Win" : "Lose";
  ...
  shutdown() {
    ...
    Network.removeListener(actions.UPDATE_SCORE);
    Network.removeListener(actions.GAME_END);
  }

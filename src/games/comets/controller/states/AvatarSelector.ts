...
export class AvatarSelector extends Phaser.State {
  ...
  public preload() {
    Network.onUsingAvatars(data => {
      this.ships = this.ships.map(character => {
        character.use = false;
        if (~data.indexOf(character.name)) {
          character.use = true;
        }
        return character;
      });
    ...
    Network.onUpdatePlayerAvatar(data => {
      this.ships = this.ships.map(character => {
    ...
    Network.getUsingAvatars({ gameId: gameId });
  }
  ...
  public create() {
    ...
    (<any>this.scrolingMap).savedPosition = new Phaser.Point(
      this.scrolingMap.x,
      this.scrolingMap.y
    );
  ...
  shutdown() {
    Network.removeListener(actions.UPDATE_PLAYER_AVATAR);
    Network.removeListener(actions.USING_AVATARS);
  }
  ...
  private actionOnClick() {
    if (!this.ships[this.selectedShipIndex].use) {
      Network.setPlayerAvatar({
        playerId: (<any>this.game.state).id,
        gameId: gameId,
        avatar: this.ships[this.selectedShipIndex].class.getName()
      });
      (<any>this.game.state).shipClass = this.ships[
